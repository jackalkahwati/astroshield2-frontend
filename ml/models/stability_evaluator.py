...
class StabilityEvaluator(BaseModel):
    ...
    def _preprocess_data(self, data: Dict[str, Any]) -> torch.Tensor:
        """Convert stability data to tensor"""
        try:
    ...
    def _postprocess_output(self, output: torch.Tensor) -> Dict[str, Any]:
        """Convert model output to stability evaluation"""
        score = output.item()
        ...
        return {
            ...
            'details': {
                'stability_level': 'stable' if score > 0.7 else 'unstable',
