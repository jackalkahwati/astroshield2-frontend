...
echo "=== Deploying Full AstroShield Application ==="
echo "This script will deploy the complete application including backend and database"

# Create the deployment script to run on the EC2 instance
cat > ec2_full_deploy.sh << 'EOF'
#!/bin/bash
set -e
...
scp ec2_full_deploy.sh astroshield:~/

# Run the script on EC2
echo "Running full deployment on EC2..."
ssh astroshield "chmod +x ~/ec2_full_deploy.sh && sudo ~/ec2_full_deploy.sh"

echo "Full application deployment completed!"
echo "You can now access the complete application with backend functionality."
echo "Keep your SSH tunnel open with:"
