...
echo "=== Fixing Backend Module Import Issue ==="
echo "This script will fix the backend Dockerfile to correctly reference the app module"

# Create the script to run on the EC2 instance
cat > ec2_fix_backend.sh << 'EOF'
#!/bin/bash
set -e
...
chmod +x ec2_fix_backend.sh
scp ec2_fix_backend.sh astroshield:~/

# Run the script on EC2
echo "Running script on EC2..."
ssh astroshield "chmod +x ~/ec2_fix_backend.sh && ~/ec2_fix_backend.sh"

echo "Backend module import issue has been fixed."
