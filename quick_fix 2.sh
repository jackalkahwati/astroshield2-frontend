...
echo "=== Simplifying Deployment ==="
echo "This script will create a simple static frontend to verify nginx works"

# Create the script to run on the EC2 instance
cat > ec2_quick_fix.sh << 'EOF'
#!/bin/bash
set -e
...
chmod +x ec2_quick_fix.sh
scp ec2_quick_fix.sh astroshield:~/

# Run the script on EC2
echo "Running script on EC2..."
ssh astroshield "chmod +x ~/ec2_quick_fix.sh && ~/ec2_quick_fix.sh"
