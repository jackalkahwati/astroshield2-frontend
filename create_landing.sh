...
echo "=== Creating a proper landing page ==="
echo "This script will create a dedicated landing page for the AstroShield site"

# Create the script to run on the EC2 instance
cat > ec2_landing_page.sh << 'EOF'
#!/bin/bash
set -e
...
chmod +x ec2_landing_page.sh
scp ec2_landing_page.sh astroshield:~/

# Run the script on EC2
echo "Running script on EC2..."
ssh astroshield "chmod +x ~/ec2_landing_page.sh && ~/ec2_landing_page.sh"
