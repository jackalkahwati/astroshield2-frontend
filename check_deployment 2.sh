...
echo "=== Checking AstroShield Deployment Status ==="
echo "This script will verify the deployment on AWS and diagnose any issues"

# Create the script to run on the EC2 instance
cat > ec2_check_status.sh << 'EOF'
#!/bin/bash
set -e
...
chmod +x ec2_check_status.sh
scp ec2_check_status.sh astroshield:~/

# Run the script on EC2
echo "Running status check on EC2..."
ssh astroshield "chmod +x ~/ec2_check_status.sh && ~/ec2_check_status.sh" 