...
export function PlanManeuverForm() {
  ...
  async function onSubmit(data: FormValues) {
    try {
      ...
      const safeData = data || {};
      if (!safeData.scheduled_date) {
        throw new Error("Please select a date");
      }
      if (!safeData.scheduled_time) {
        throw new Error("Please select a time");
      }
      
      // Combine date and time into a proper datetime
      const scheduledDateTime = new Date(safeData.scheduled_date);
      const timeString = safeData.scheduled_time || "12:00"; // Fallback to noon if time is empty
      ...
      const maneuverData = {
        satellite_id: safeData.satellite_id,
        type: safeData.type,
        scheduled_time: scheduledDateTime.toISOString(),
        parameters: {
          delta_v: safeData.delta_v,
          burn_duration: safeData.burn_duration,
          direction: {
            x: safeData.direction_x,
            y: safeData.direction_y,
            z: safeData.direction_z
          },
          target_orbit: null  // Optional field
        },
        priority: safeData.priority,
        notes: safeData.notes || undefined
      }
      ...
      toast({
        ...
        description: `${maneuverTypes.find(t => t.value === safeData.type)?.label} scheduled for ${format(scheduledDateTime, "PPP 'at' p")}`,
      })
      ...
      <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                    ...
                    <FormItem>
                      ...
                      <FormControl>
                        <Input
                          ...
                          step="0.001"
                          value={field.value || 0}
                          onChange={e => field.onChange && field.onChange(parseFloat(e.target.value) || 0)}
                        />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                ...
                render={(renderProps) => {
                  ...
                  return (
                    <FormItem>
                      ...
                      <Select onValueChange={(value) => field.onChange && field.onChange(parseInt(value))} value={field.value?.toString() || "3"}>
                        <FormControl>
                          <SelectTrigger>
            ...
            <div className="space-y-2">
              <FormLabel>Direction Vector</FormLabel>
              <div className="grid grid-cols-3 gap-4">
                <FormField
                  control={form.control}
                  name="direction_x"
