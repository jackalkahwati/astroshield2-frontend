...
const TrajectoryAnalysis: React.FC = () => {
  ...
  const [trajectoryData, setTrajectoryData] = useState<TrajectoryResult | null>(null);
  ...
  const handleAnalyze = async () => {
    ...
    try {
      // Use the local backend API with correct port and path
      const apiUrl = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5002';
      const response = await fetch(`${apiUrl}/api/trajectory/analyze`, {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          config: {
            ...
            object_properties: {
              ...
              area: config.object_properties.area,
              cd: config.object_properties.cd
            },
            breakup_model: {
              enabled: config.breakup_model.enabled,
              fragmentation_threshold: config.breakup_model.fragmentation_threshold
            }
          },
          initial_state: [
            initialState[0], // longitude
            initialState[1], // latitude
            initialState[2], // altitude
            initialState[3], // velocity x
            initialState[4], // velocity y
            initialState[5]  // velocity z
          ]
        })
      });
      ...
      if (!response.ok) {
        ...
        let errorMessage = 'Could not connect to analysis server.';
        try {
      ...
      console.error('Error in trajectory analysis:', err);
        ...
        <CardContent>
          <Tabs value={activeTab} onValueChange={setActiveTab}>
            <TabsList className="grid w-full grid-cols-2">
              ...
              <TabsTrigger value="results" disabled={!trajectoryData}>Results</TabsTrigger>
            </TabsList>
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="space-y-1">
                        <Label htmlFor="cd">Drag Coefficient</Label>
                        <div className="flex gap-4 items-center">
                          <Slider 
                            id="cd"
                            ...
                            max={5}
                            step={0.1}
                            onValueChange={(values) => handleNestedChange('object_properties', 'cd', values[0])}
                            className="flex-1"
                          />
                          <Input 
                            type="number"
                            className="w-24"
                            value={config.object_properties.cd}
                            onChange={(e) => handleNestedChange('object_properties', 'cd', parseFloat(e.target.value))}
                          />
                        </div>
                      </div>
                    </div>
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="space-y-1">
                        <Label htmlFor="monte_carlo_samples">
                          Monte Carlo Samples
                        </Label>
                        <div className="flex gap-4 items-center">
                          <Slider 
                            ...
                            max={1000}
                            step={1}
                            onValueChange={(values) => handleChange('monte_carlo_samples', values[0])}
                            className="flex-1"
                          />
                          <Input 
                            type="number"
                            className="w-24"
                            value={config.monte_carlo_samples}
                            onChange={(e) => handleChange('monte_carlo_samples', parseInt(e.target.value))}
                          />
                        </div>
                      </div>
                      ...
                      <div className="flex items-center space-x-2 pt-2">
                        <Switch 
                          id="breakup_enabled"
                          checked={config.breakup_model.enabled}
                          onCheckedChange={(checked) => handleNestedChange('breakup_model', 'enabled', checked)}
                        />
                        <Label htmlFor="breakup_enabled">Enable Breakup Modeling</Label>
                      </div>
                      
                      {config.breakup_model.enabled && (
                        <div className="space-y-1 pl-6 pt-1">
                          <Label htmlFor="fragmentation_threshold">
                            Fragmentation Threshold (kPa)
                          </Label>
                          <div className="flex gap-4 items-center">
                            <Slider 
                              id="fragmentation_threshold"
                              value={[config.breakup_model.fragmentation_threshold]}
                              min={10}
                              max={200}
                              step={1}
                              onValueChange={(values) => handleNestedChange('breakup_model', 'fragmentation_threshold', values[0])}
                              className="flex-1"
                            />
                            <Input 
                              type="number"
                              className="w-24"
                              value={config.breakup_model.fragmentation_threshold}
                              onChange={(e) => handleNestedChange('breakup_model', 'fragmentation_threshold', parseInt(e.target.value))}
                            />
                          </div>
                        </div>
                      )}
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div className="space-y-1">
                        <Label htmlFor="velocity_y">Velocity Y (m/s)</Label>
                        <Input 
                          id="velocity_y"
                          type="number"
                          value={initialState[4]}
                          onChange={(e) => handleInitialStateChange(4, parseFloat(e.target.value))}
                        />
                      </div>
                      
                      <div className="space-y-1">
                        <Label htmlFor="velocity_z">Velocity Z (m/s)</Label>
                        <Input 
                          id="velocity_z"
                          type="number"
                          value={initialState[5]}
                          onChange={(e) => handleInitialStateChange(5, parseFloat(e.target.value))}
                        />
                      </div>
                    </div>
                  </div>
