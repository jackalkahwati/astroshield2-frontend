...
echo "=== Fixing EC2 Network Configuration ==="
echo "This script will address the network connectivity issues"

# Create the script to run on the EC2 instance
cat > ec2_network_fix.sh << 'EOF'
#!/bin/bash
set -e
...
chmod +x ec2_network_fix.sh
scp ec2_network_fix.sh astroshield:~/

# Run the script on EC2
echo "Running network fix script on EC2..."
ssh astroshield "chmod +x ~/ec2_network_fix.sh && sudo ~/ec2_network_fix.sh"

echo "Network fixes have been applied."
echo "Since this is a private EC2 instance without a public IP address, you'll need to:"
echo "1. Access the application through a bastion host or AWS SSM"
echo "2. Set up proper VPC networking with a NAT Gateway"
echo "3. Configure an Application Load Balancer to expose the application publicly" 