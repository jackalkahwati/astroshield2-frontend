...
echo "=== Creating a direct landing page ==="
echo "This script will create a landing page directly in the frontend container"

# Create the script to run on the EC2 instance
cat > ec2_direct_landing.sh << 'EOF'
#!/bin/bash
set -e

echo "=== Creating direct landing page ==="

# Create a temporary landing page file
cat > index.html << 'EOT'
<!DOCTYPE html>
<html>
<head>
...
# Create a simple frontend container with the landing page
echo "Creating a simple static container..."
cd /home/stardrive/astroshield/deployment

# Stop the existing containers
sudo docker-compose down

# Create a simple container that serves a static file
mkdir -p simple-static
cp index.html simple-static/
...
chmod +x ec2_direct_landing.sh
scp ec2_direct_landing.sh astroshield:~/

# Run the script on EC2
echo "Running script on EC2..."
ssh astroshield "chmod +x ~/ec2_direct_landing.sh && ~/ec2_direct_landing.sh"

echo "Direct landing page has been created and deployed."
